...
> **Status: A2 deliverable, v0.2 (May 2026).** Supersedes v0.1. Brings forward v0.1's structural skeleton (Alignment / Conflict / Additive / Ours-Does-Better / Theory naming / template mapping table / proposed Decisions / open questions / V1 implications / closing) and updates throughout for the post-Phase-2 canon — committed Decisions #42–#47, Bible §2.3 and §6.3 cross-mode reframes, Doc 02 §5.1 cross-mode framing, Doc 09 §1.2 (user-modeling layer pillar), §6A (Agent Control Surfaces per Content Mode), §13.11 (Content Classes Beyond the Core Three), §16 (Cost Considerations), the Glossary's new Interactive Tutorial entry, and Doc 10's 5 demo-relevant goal categories. Proposed Decisions renumber from v0.1's #42–#46 to #48–#52, since #42–#47 are now committed canon.
>
...
6. **Patrick's "Game Mode vs. Free Play" distinction is still additive, but its architectural home shifts in v0.2.** Free Play (no pass/fail, indefinite practice with stats tracked) is best framed as a setting within the **exercise control surface** (Doc 09 §6A.2) — specifically a completion-policy element — rather than as a new top-level `session_mode` schema enum. This is more economical and consistent with the §6A.2 architecture. Decision #45 has already formalized the analogous capability ("no completion criteria, just play endlessly") for user-generated sight-reading. See §5.1.
...
Patrick's "Recommended for You" carousel (§2.3) on the Exercise Section landing page maps to our **Hook zone** in Screen A (UX Spec §3.1). The framing is identical at the surface level, even if our canon lacks Patrick's specific carousel-card design. **v0.2 strengthening:** with Decision #47 elevating the user-modeling layer to a vision pillar, the Hook zone is now architecturally significant — it's the most user-visible affordance for the user-model's outputs. Patrick's specific carousel-card design is a candidate UI for that surface.
...
### 5.1 Free Play mode within exercises *(architectural placement revised in v0.2)*
...
**v0.2 reframe.** v0.1 proposed adding a `session_mode` enum to the Exercise Instance / Result schemas. Post-Phase-2, a more economical placement is available: the **exercise control surface** (Doc 09 §6A.2) explicitly lists "performance constraints" and "configurable assistance" as enumerable surface elements. Adding a **completion policy** element to that surface — with values like `practice` (default; session contributes to mastery) and `free_play` (non-assessable; no mastery contribution; stats still tracked) — handles Patrick's distinction without introducing a parallel schema axis. Decision #45 already established the analogous capability ("no completion criteria, just play endlessly") within user-generated sight-reading; this is the exercise-side parallel.

This connects to:
- **Candidate content mode "Free Play"** (Bible §2.1.1) — which is the *original* sense: open-ended musical improvisation without notation guidance. Decision #45 explicitly clarified this remains deferred indefinitely. Patrick's "Free Play within an exercise atom" is a different thing from Free Play original-sense — it's a completion-policy setting, not a content mode.
- **Staley's "free-play interlude as part of the path"** comment (Standup Synthesis §1) — likely refers to this same completion-policy setting being invocable as part of a Project's roadmap.
- **Andrew's Free Play mode** in the repertoire editor demo — analogous setting for repertoire.

**Recommendation:** Absorb Patrick's Free Play as a **completion-policy element of the exercise control surface** (Doc 09 §6A.2), not as a standalone `session_mode` schema enum. The exercise control surface already exists as the locus for this kind of session-level configuration; adding completion policy there is the smallest change with the most reusability. Worth a candidate Decision (see §10, proposed Decision #49). The same shape generalizes to repertoire (where it likely already exists under Andrew's editor) and is the parallel of Decision #45 for sight reading.
...
### 5.4 Concrete mastery thresholds *(strengthened in v0.2)*
...
2. **Glossary "Content Mode"**: Update to remove Theory from the candidate list (currently reads "Theory and Free Play are candidate future content modes (status TBD)"). Free Play (original sense) stays as candidate per Decision #45's clarification; Theory resolves.
...
**Net for V1 catalog**: ~22 templates from Patrick survive direct or modified absorption. Most map to atoms we'd generate from the framework anyway; what Patrick adds is concrete content scope examples, passing criteria, and (newly important) the demo-aligned genre/style content scope.

### 8.3 Passing criteria absorption *(framing updated in v0.2)*

Patrick's passing criteria are remarkably consistent across templates. Extract these as default `mastery_thresholds` patterns — which, per Decision #43, are the exercise-section instantiation of the cross-mode performance-constraints primitive:
...
These become the `mastery_thresholds` defaults for V1 catalog atoms; per-atom values during catalog authoring can override. Worth flagging as candidate Decision #50 (see §10). Same shape will inform sight-reading and repertoire performance-constraint defaults as those modes' instantiation work proceeds (Decision #43, Doc 02 §5.1 cross-mode framing).
...
### 9.5 Free Play is small but real, and its V1 placement is the exercise control surface *(revised in v0.2)*
§5.1 Free Play absorption is small and worth doing in V1, but the placement revision matters: it's a completion-policy element of the exercise control surface (Doc 09 §6A.2), not a standalone `session_mode` schema enum. This is more economical, parallels Decision #45's resolution for sight-reading, and uses the existing §6A architecture rather than introducing a new schema axis.
...
4. Glossary "Content Mode" entry updates to remove Theory from the candidate list. Free Play (original sense) remains a candidate per Decision #45's clarification.
5. Doc 09 §13.11 #51 (Content Classes Inventory) marks Theory as resolved by this Decision. #52 (Interactive Tutorial placement) and #53 (Video Library placement) remain open within the cluster.
...
**Cross-references:** Bible §2.1.1, Glossary "Content Mode," Decision #41, Decision #45 (Free Play status preserved), Doc 09 §13.11 (cluster home).

---

### Proposed Decision #49: Free Play as Completion-Policy Element of the Exercise Control Surface *(renumbered and reframed from v0.1's #43)*

**Source:** A2 evaluation of Patrick's Game Mode vs. Free Play distinction (his §2.1).

**Decision proposed:** Patrick's "Free Play within an exercise atom" — practice an atom indefinitely with no pass/fail, no mastery contribution, but stats still tracked — is added as a **completion-policy element of the exercise control surface (Doc 09 §6A.2)**, not as a standalone `session_mode` schema enum. The surface element takes two values:
...
This placement parallels Decision #45's "no completion criteria, just play endlessly" option for user-generated sight-reading; both are completion-policy settings within their respective mode's control surface, not new content modes. Patrick's Free Play is distinct from Free Play original-sense (Bible §2.1.1 candidate content mode — open-ended improvisation without notation guidance), which remains deferred indefinitely per Decision #45's clarification.

**Reasoning:** Free Play is a real practice need (warm-up, experimentation, indefinite practice without pass/fail pressure) that the current schema doesn't support. Placing it in the exercise control surface (an architecture that already exists per Decision #46 and Doc 09 §6A.2) is more economical than introducing a new schema axis, and parallels how the analogous capability is handled in sight reading (Decision #45). The control surface architecture is the right home for session-level configuration the user or agent can manipulate.

**Phasing:** V1. The control surface element addition is non-breaking; the UI surface for Free Play is a small Screen B / Screen C addition.

**Cross-references:** Doc 09 §6A.2 (exercise control surface), Decision #46 (control surface enumeration), Decision #45 (parallel for sight-reading; clarifies Free Play original-sense remains deferred), Bible §2.1.1 (Free Play original-sense candidate content mode — status unchanged), Standup Synthesis §1 (Staley's "free-play interlude as part of the path"), Architecture Doc §8.

---

### Proposed Decision #50: Default Mastery Thresholds From Patrick's Patterns *(renumbered from v0.1's #44)*
...
**Reasoning:** Patrick's templates are remarkably consistent; the patterns are defensible defaults rather than guessing. They satisfy the "explicit clearance criteria" requirement of Decision #40 with a starting point. Per-atom values during catalog authoring can override. Cross-mode framing per Decision #43 means these defaults seed the larger performance-constraint vocabulary, not just the exercise-section schema.

**Phasing:** V1, applied during catalog authoring (Track D in the A1→PRD plan).

**Cross-references:** Decision #40 (`mastery_thresholds` field), Decision #43 (cross-mode performance constraints), Bible §2.3 (cross-mode primitive framing), Doc 02 §5.1 (cross-mode tech spec scope), Architecture Doc §8.1.
...
Decision #13 defers; this question survives the deferral. **v0.2 cross-ref:** this question is now adjacent to Doc 09 §13.11's "Content Classes Beyond the Core Three" cluster, though it's not yet listed there explicitly. If Technique becomes a candidate distinct content class, it would join the cluster.
...
**Net absorption from the brainstorm doc:**
- **~22 of 50 exercise templates** survive direct or modified absorption as concrete V1 catalog instantiations
- **5 default mastery-threshold patterns** extracted from Patrick's passing criteria, now framed as the exercise-section instantiation of cross-mode performance constraints (Decision #43)
- **Free Play (within atoms)** absorbed as a completion-policy element of the exercise control surface (proposed Decision #49, parallel to Decision #45's sight-reading resolution)
...
- **Knowledge Library** (renamed Theory Library) deferred beyond V1 (proposed Decision #51, formal tracking under Doc 09 §13.11 #53)
- **Patrick's H1 Alberti bass example** confirmed as direct demo-relevant content per Doc 10 §5.4 goal category 3
- **8 open questions** added to the queue for B-track triage (one new in v0.2: the premium-tier-vs-§16 tension)
